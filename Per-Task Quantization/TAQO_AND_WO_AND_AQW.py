...
# -*- coding: utf-8 -*-
"""
Task-Aware Quantization on TriviaQA (AQW, TAQO, WO)
- Models: restricted to Llama* and Qwen* families
- Methods:
        1) AQW   -> AutoAWQ, all transformer blocks quantized (mixed calibration)
        2) TAQO  -> Task-Aware Quantization, keep top-K + first/last FP16 (Trivia-only calib)
        3) WO    -> Weight-Only baseline (original FP16 model)

Evaluation: greedy decode; TriviaQA EM/F1 (SQuAD normalization + aliases)
Install (one-time, GPU):
    pip install -U "transformers==4.51.3" "torch==2.6.*" datasets accelerate autoawq

srun example:

    srun -c 4 -A tdk -p tdk --gres=gpu:1 --pty  python TAQO_AND_WO_AND_AQW.py \
...
# BitsAndBytes baselines removed in this slim runner (keep only AQW/TAQO/WO)
...
METHODS = [
    "AQW",   # AutoAWQ all layers, mixed calibration
    "TAQO",  # Task-Aware Quantization (per-layer keep) on Trivia-only calib
    "WO",    # Weight-Only baseline: unquantized FP16
]
...
## record_custom_bit_allocation removed (only AQW/TAQO/WO remain)
...
def run(args):
    ...
    for model_name in args.models:
        ...
        for method in args.methods:
            ...
            if ml == "aqw":
                # Quantize ALL layers with AWQ (no FP16-kept subset) - using MIXED calibration
                q_model, q_tok, _ = awq_quantize_from_texts(model_name, tokenizer, mixed_texts, keep_layers=None, args=args, device=device)
                em, f1, n, _ = evaluate_triviaqa("AQW", model_name, q_model, q_tok, eval_data, args.gen_len, args.save_directory, device)
                results_rows.append((model_name, "AQW", em, f1, n))
                try:
                    record_bit_allocation(model_name, "AQW", base_model, keep_layers=None, awq_wbit=args.awq_wbit, save_directory=args.save_directory)
                except Exception:
                    pass
                ...
                em, f1, n, _ = evaluate_triviaqa("TAQO", model_name, q_model, q_tok, eval_data, args.gen_len, args.save_directory, device)
                results_rows.append((model_name, "TAQO", em, f1, n))
                try:
                    record_bit_allocation(model_name, "TAQO", base_model, keep_layers=keep, awq_wbit=args.awq_wbit, save_directory=args.save_directory)
                except Exception:
                    pass

            elif ml == "wo":
                # Evaluate original FP16 model as-is
                em, f1, n, _ = evaluate_triviaqa("WO", model_name, base_model, tokenizer, eval_data, args.gen_len, args.save_directory, device)
                results_rows.append((model_name, "WO", em, f1, n))
